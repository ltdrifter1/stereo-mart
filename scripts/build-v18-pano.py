#!/usr/bin/env python3
"""
V18 panorama production bake — Stereo-Mart PNW warehouse editorial.

Input : art/stereo-mart-equirect-warehouse-v18.png  (4096×2048 industrial shop)
Output: public/textures/store_pano_v18.webp         4096x2048 lights-on
        public/textures/store_pano_off_v18.webp     4096x2048 lights-off grade
        public/textures/store_pano_lqip_v18.webp    512x256 preview
        public/textures/store_pano_fg_v18.webp      transparent FG parallax
        public/textures/store_pano_mg_v18.webp      transparent MG parallax
...
Run: python3 scripts/paint-stereo-mart-v18-warehouse.py
...
SRC = ROOT / "art" / "stereo-mart-equirect-warehouse-v18.png"
SRC_FG = ROOT / "art" / "layers" / "warehouse-fg-v18.png"
SRC_MG = ROOT / "art" / "layers" / "warehouse-mg-v18.png"
OUT_ON = ROOT / "public" / "textures" / "store_pano_v18.webp"
OUT_OFF = ROOT / "public" / "textures" / "store_pano_off_v18.webp"
OUT_LQIP = ROOT / "public" / "textures" / "store_pano_lqip_v18.webp"
OUT_FG = ROOT / "public" / "textures" / "store_pano_fg_v18.webp"
OUT_MG = ROOT / "public" / "textures" / "store_pano_mg_v18.webp"
OUT_PNG = ROOT / "public" / "textures" / "store_pano_v18.png"
...
PLANES = {
    # v18 warehouse: LISTEN headphones + turntable on back wall
    "listening-booth": (0.500, 0.400, 16.0, 18.0),
    # CRT on cabinet by night dock windows
...
CRT_FILE_BOX_NORM = (0.02, 0.42, 0.20, 0.72)  # v18 CRT cabinet
...
def bt_grade(pano: Image.Image) -> Image.Image:
    """Cool editorial polish — clarity without cartoon warmth."""
    a = np.asarray(pano).astype(np.float32) / 255.0
    lifted = np.clip(a * 1.04 + 0.01, 0.0, 1.0)
    contrasted = np.clip(0.5 + (lifted - 0.5) * 1.1, 0.0, 1.0)
    contrasted[..., 0] = np.clip(contrasted[..., 0] * 0.98, 0.0, 1.0)
    contrasted[..., 1] = np.clip(contrasted[..., 1] * 1.01, 0.0, 1.0)
    contrasted[..., 2] = np.clip(contrasted[..., 2] * 1.04, 0.0, 1.0)
...
def main() -> None:
    pano = bake_pano()
    pano.save(OUT_ON, "WEBP", quality=88, method=6)
